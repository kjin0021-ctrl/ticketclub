...
test("builds a chronological Korea trip around the confirmed artist event", () => {
  const days = buildKoreaTripPlan({ event, flight, venueArrivalAt: "2026-08-29T03:45:00Z", immigrationMinutes: 120, lodging: "Hongdae guesthouse", assumedReturnHomeAt: "2026-09-01T20:00:00Z", nearbyEvents: [{ id: "nearby-1", title: "Inkigayo", startsAt: "08.30 · 15:20", distanceKm: 4.2, fit: "on-route" }], includeNearbyIds: ["nearby-1"] });
...
test("calendar export contains valid event boundaries and escaped content", () => {
  const days = buildKoreaTripPlan({ event, flight, venueArrivalAt: "2026-08-29T03:45:00Z", immigrationMinutes: 120, lodging: "Hotel, Seoul", assumedReturnHomeAt: "2026-09-01T20:00:00Z" });
...
test("verified revisit spots replace the generic personal-place placeholder", () => {
  const days = buildKoreaTripPlan({ event, flight, venueArrivalAt: "2026-08-29T03:45:00Z", immigrationMinutes: 120, lodging: "", assumedReturnHomeAt: "2026-09-01T20:00:00Z", personalSpots: [{ id: "spot-1", name: "My favorite cafe", address: "Hongdae 12", suitableTime: "演出结束后仍营业" }] });
  const items = days.flatMap((day) => day.items);
